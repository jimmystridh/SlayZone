...
h.db
  ...
  .run(projectId, 'P', '#000', '/tmp/test-proj')
...
const item2 = h.invoke('ai-config:create-item', {
  type: 'skill',
  scope: 'library',
  slug: 'sel-test-2',
  content: 'y'
...
describe('ai-config:set-project-selection', () => {
  test('creates selection and canonicalizes legacy claude skill paths', () => {
    ...
    const sels = h.invoke('ai-config:list-project-selections', projectId) as {
      item_id: string
      target_path: string
      provider: string
    }[]
  ...
  test('supports provider-specific upserts', () => {
    ...
    const sels = h.invoke('ai-config:list-project-selections', projectId) as Array<{
      provider: string
      target_path: string
    }>
...
describe('ai-config:list-providers', () => {
  test('returns seeded providers', () => {
    const providers = h.invoke('ai-config:list-providers') as {
      name: string
      kind: string
      enabled: number
    }[]
