...
const DIFF_CONTENT: DiffLine[] = [
  ...
  { old: undefined, new: 5, text: '+', type: 'add' },
  {
    old: undefined,
    new: 6,
    text: '+export function authMiddleware(req, res, next) {',
    type: 'add'
  },
  {
    old: undefined,
  ...
  { old: undefined, new: 8, text: '+  if (!token) {', type: 'add' },
  {
    old: undefined,
    new: 9,
    text: "+    res.status(401).json({ error: 'Unauthorized' })",
    type: 'add'
  },
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
      ...
      <div>
        ...
        <div className="mt-1.5 h-10 rounded border bg-muted/20 flex items-center gap-2 px-3">
          <span className="text-[13px] font-semibold text-green-500 bg-green-500/10 px-2 py-0.5 rounded">
            1 staged
          </span>
          <span className="text-[13px] font-semibold text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded">
            3 modified
          </span>
          <span className="text-[13px] text-muted-foreground/60 bg-muted px-2 py-0.5 rounded">
            1 untracked
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mt-1.5 rounded border bg-muted/20 overflow-hidden">
          {COMMITS.map((c, i) => (
            <div
              key={c.hash}
              className={`flex items-start gap-3 px-3 py-2.5 ${i > 0 ? 'border-t border-border/50' : ''}`}
            >
          ...
          {FILE_TREE.map((f, i) => (
            ...
            >
              {'folder' in f ? (
                ...
                <>
                  <span
                    className={`font-bold w-3 shrink-0 ${f.status === '?' ? 'text-green-500' : 'text-yellow-500'}`}
                  >
                    {f.status}
                  </span>
                  <span className="truncate flex-1">{f.name}</span>
                  <span className="text-green-500 shrink-0">+{f.added}</span>
                  {'removed' in f && f.removed > 0 && (
                    <span className="text-red-400 shrink-0">-{f.removed}</span>
                  )}
                </>
              )}
            </div>
          ...
          {DIFF_CONTENT.map((line, i) => {
            ...
            return (
              <div key={i} className={`flex ${bg} whitespace-pre`}>
                <span className="w-8 shrink-0 text-right pr-1 text-muted-foreground/25 select-none">
                  {line.old ?? ''}
                </span>
                <span className="w-8 shrink-0 text-right pr-2 text-muted-foreground/25 select-none">
                  {line.new ?? ''}
                </span>
  ...
  return (
    ...
    >
      <div className="h-12 shrink-0 border-b flex items-center px-4 gap-1">
        <div
          className={`h-8 px-3 rounded text-[14px] font-medium flex items-center ${!diffActive ? 'bg-muted text-foreground' : 'text-muted-foreground/60'}`}
        >
          General
        </div>
        <div
  ...
  return (
    <SceneShell tabs={[{ label: 'Set up auth', active: true, dot: true }]}>
      <TaskHeader
        ...
        panels={panelButtons('Terminal', ...(showGit ? (['Git'] as const) : []))}
      />

      <div className="flex-1 flex gap-4 p-4 min-h-0 overflow-hidden">
        {/* Terminal — full width initially, shrinks when git opens */}
        <div
          className={`${showGit ? 'w-2/5' : 'flex-1'} shrink-0 rounded-xl border overflow-hidden flex flex-col bg-neutral-950 transition-all duration-300`}
        >
        ...
        <AnimatePresence>{showGit && <GitPanel diffActive={diffActive} />}</AnimatePresence>
      </div>
      <AnimatedCursor
        waypoints={[
