    ...
    <div className="flex-1 flex flex-col min-w-0">
      <div className="flex items-center justify-between px-1.5 mb-1.5 select-none">
        <div className="flex items-center gap-2">
          <h3
            className={cn(
              'text-[10px] font-semibold text-muted-foreground transition-colors',
              flash && 'text-primary'
            )}
          >
            {title}
...
const INITIAL_IN_PROGRESS: MockTask[] = [{ id: 'kanban', title: 'Build kanban board', priority: 1 }]
const DONE: MockTask[] = [{ id: 'landing', title: 'Landing page', priority: 4 }]
    ...
    <div className="w-full h-full flex flex-col rounded-xl border bg-surface-0 shadow-xl overflow-hidden">
      ...
      <div className="h-9 shrink-0 flex items-center gap-1 px-2 bg-neutral-100 dark:bg-neutral-900/80">
        ...
        {['Set up auth', 'Build API'].map((title) => (
          <div
            key={title}
            className="h-6 px-2.5 rounded-md flex items-center gap-1.5 text-neutral-500 dark:text-neutral-400 bg-neutral-100 dark:bg-neutral-800/50 shrink-0"
          >
      ...
      <LayoutGroup>
        <div className="flex-1 flex gap-3 p-3 overflow-hidden">
          ...
          />
          <MockColumn title="In Progress" tasks={inProgress} flash={flash} />
          <MockColumn title="Done" tasks={DONE} />
        </div>
      </LayoutGroup>
    </div>
  )
